...
object LuceMro {
  case class Mro(objid: String, scpci: String, scrsrp: Double, ncci: String, ncpci: String, ncrsrp: Double, lon: Double, lat: Double,
                  timestamp: Long, s1apid: String, host: String, cover_type: String, region_name: String)
                  
  case class MroLuce(objid: String, scrsrp: Double, timestamp: String, s1apid: String, lon: Double, lat: Double, ncci_list: String, ncrsrp_list: String)                
  
  case class TrainingMro(objid: String, scrsrp: Double, ncci: String, ncrsrp: Double, lon: Double, lat: Double)
  
  def rddToMroLuceDFCase(sparkSession : SparkSession, path:String):Dataset[MroLuce] = {
    val rowsRDD = sparkSession.sparkContext.textFile(path)
    val mroRDD = rowsRDD.map(row => row.split(",")).filter(_.size == 8)
                    .map{col => MroLuce(col(0),col(1).toDouble,col(2),col(3),col(4).toDouble,col(5).toDouble,col(6),col(7))}
  ...
  def rddToMroDFCase(sparkSession : SparkSession, path:String):Dataset[Mro] = {
    val rowsRDD = sparkSession.sparkContext.textFile(path)
    val mroRDD = rowsRDD.map(row => row.split(",")).filter(_.size == 13)
                    .map{col => Mro(col(0),col(1),col(2).toDouble,col(3),col(4),col(5).toDouble,col(6).toDouble,
                        col(7).toDouble,col(8).toLong,col(9),col(10),col(11),col(12))}
  ...
  def rddToTrainingMroDFCase(sparkSession : SparkSession, path:String):Dataset[TrainingMro] = {
    val rowsRDD = sparkSession.sparkContext.textFile(path)
    val mroRDD = rowsRDD.map(row => row.split(",")).filter(_.size == 7)
...
//    mroDS.createOrReplaceTempView("mro_testdata")
    ...
    //val mrotest1DF = sparkSession.sql("select objid,scrsrp,timestamp,s1apid,substr(rpad(lon,8,'0'),1,8) as lon,substr(rpad(lat,7,'0'),1,7) as lat,concat_ws('-', collect_list(ncci)) as ncci_list,concat_ws('-', collect_list(cast(ncrsrp as string))) as ncrsrp_list from mro_testdata group by objid,scrsrp,timestamp,s1apid,substr(rpad(lon,8,'0'),1,8),substr(rpad(lat,7,'0'),1,7)")
    //mrotest1DF.show(false)
    
    val mrotest1DF = rddToMroLuceDFCase(sparkSession,testdataDir)
    
    val mrotest1DFfilternull = mrotest1DF.filter(!mrotest1DF("ncci_list").contains("null")) 
...
//    objid2grid.show(3,false)
    ...
    val mrotest_objid2grid_mrotrain = sparkSession.sql("select t1.objid,t1.scrsrp,t1.timestamp,t1.s1apid,t1.lon,t1.lat,t1.ncci_list as test_nci_list,t1.ncrsrp_list as test_ncirsrp_list,t2.lonlat,t2.scrsrp_list,t2.ncci_list,t2.ncrsrp_list"+
                                      " from mro_testdata_combine t1 inner join mro_traindata_combine t2 on t1.objid = t2.objid")
    mrotest_objid2grid_mrotrain.persist();     
    mrotest_objid2grid_mrotrain.createOrReplaceTempView("jointab1")
    println("----------------------------mrotest_objid2grid_mrotrain>>>>>>>>>>>>>"+mrotest_objid2grid_mrotrain.count()+"<<<<----"+sparkSession.sql("select distinct objid,scrsrp,timestamp,s1apid from jointab1").count())
    ...
    res.cache()
    res.createOrReplaceTempView("jointab2")
    println("----------------------------"+sparkSession.sql("select distinct objid,scrsrp,timestamp,s1apid from jointab2").count()+"-----")
    ...
    val res3 = res2.select(col("objid"), col("scrsrp"), col("timestamp"), col("s1apid"), col("lon"), col("lat"), col("lonlat"), col("sim"), col("lonlat_class"))
    //udaf http://bihell.com/2017/02/10/working-with-udfs-in-apache-spark/ http://blog.csdn.net/kwu_ganymede/article/details/50462020
    ...
    //res3.show(100,false)
    val res4 = res3.groupBy(col("objid"), col("scrsrp"), col("timestamp"), col("s1apid"), col("lon"), col("lat")).agg(classify(col("lonlat"), col("sim")).as("location_list"))
    //res4.show(100,false)
