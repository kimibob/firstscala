...
//    val conf = new SparkConf().setAppName("Spark MRO Location APP")
    ...
    val sql="select a.objid, concat(rpad(a.lon,8,'0'),'-',rpad(a.lat,7,'0')) as lonlat, b.scrsrp_avg, concat_ws('-', collect_list(ncci)) as ncci_list, concat_ws('-', collect_list(cast(ncrsrp as string))) as ncrsrp_list from (select lon,lat,objid,ncci,round(avg(ncrsrp),1) as ncrsrp from mro_traindata where ncci!='null' group by lon,lat,objid,ncci) a left join (select lon,lat,objid,round(avg(scrsrp),1) as scrsrp_avg from mro_traindata where ncci!='null' group by lon,lat,objid) b on a.lon=b.lon and a.lat=b.lat and a.objid=b.objid group by a.lon,a.lat,a.objid,b.scrsrp_avg "
    val mrotrain1DF = sparkSession.sql(sql)
...
//    objid2grid.show(3,false)
    ...
    val mrotest_objid2grid_mrotrain = sparkSession.sql("select t1.objid,t1.scrsrp,t1.timestamp,t1.s1apid,t1.lon,t1.lat,t1.ncci_list as test_nci_list,t1.ncrsrp_list as test_ncirsrp_list,t2.lonlat,t2.scrsrp_avg,t2.ncci_list,t2.ncrsrp_list"+
    ...
    mrotest_objid2grid_mrotrain.show(300,false)
    ...
    val similarity = udf {(objid:String, scrsrp:String,  test_nci_list:String, test_ncirsrp_list:String, lonlat:String, scrsrp_avg:String, ncci_list:String, ncrsrp_list:String) => {
...
//        }
    ...
    //res.show(false)
    res.cache()
    val res1 = res.withColumn("sim",similarity(col("objid"), col("scrsrp"), col("test_nci_list"), col("test_ncirsrp_list"), col("lonlat"), col("scrsrp_avg"), col("ncci_list"), col("ncrsrp_list")))
