...
object MROLocationInMemory {
  ...
  case class TrainingMro(objid: String, lonlat: String, scrsrp: Double, ncci_list: String, ncrsrp_list: String)
  ...
  def rddToMroDFCase(sparkSession : SparkSession, path:String):Dataset[Mro] = {
    ...
    val mroRDD = rowsRDD.map(row => row.split(",")).filter(_.size == 13)
                    .map{col => Try(
                        Mro(col(0),col(1),col(2).toDouble,col(3),col(4),col(5).toDouble,col(6).toDouble,
  ...
  def similarity(objid:String, scrsrp:Double,  test_nci_list:String, test_ncirsrp_list:String, lonlat:String, 
      scrsrp_avg:Double, ncci_list:String, ncrsrp_list:String): Double = {
        ...
        train_mrorsrp_vector(0) = scrsrp_avg
...
//    val conf = new SparkConf().setAppName("Spark MRO broadcast").setMaster("local[2]")
    ...
    val traindataRDD = sparkSession.sparkContext.textFile(traindataDir).collect()
                    .map(row => row.split(","))
                    .map{col => TrainingMro(col(0),col(1),col(2).toDouble,col(3),col(4))}
                    .map{case mro => mmap.addBinding(mro.objid, mro)}
    val broadcastVar = sparkSession.sparkContext.broadcast(mmap)
    
    println("11mro wanyou data -->>broadcast size :"+SizeEstimator.estimate(broadcastVar.value))
   ...
   * */
    val res = mrotest1DFfilternull.rdd.mapPartitions(
          iter  => {
            val traindata_boardcast = broadcastVar.value
            //objid, scrsrp, timestamp, s1apid, lon, lat, lonlat
            val arrayBuffer = ArrayBuffer[(String, Double, Long, String, String, String, String)]()
            iter.foreach{//.map
              case row =>{
                ...
                if(traindata_boardcast.getOrElse(row.getString(0),null)!=null){
                  for(train_mro <- traindata_boardcast.getOrElse(row.getString(0),null)){
                    if(train_mro != null){
                        val train_ncilist = train_mro.ncci_list.split("-").toSet
                        val testncilist = row.getString(6).split("-").toSet
                        //println(train_ncilist.toString()+"---"+testncilist.toString()+"---"+testncilist.subsetOf(train_ncilist).toString())
      ...
      //      scrsrp_avg:Double, ncci_list:String, ncrsrp_list:String):
                          val sim = similarity(row.getString(0), row.getDouble(1), row.getString(6), row.getString(7), train_mro.lonlat, 
      ...
      //                        , train_mro.lonlat, sim, grid_tag))
                          ...
                          mm.addBinding(grid_tag, train_mro.lonlat+"|"+sim)
                        }
                    }
                  }
                }
