 ...
 * payload数据定位
 */
object MROLocation_payload {
  case class Mro(timestamp: String, s1apid: String, objid: String, scrsrp: String,
      ncci1: String, nc1rsrp: String, ncci2: String, nc2rsrp: String,
      ncci3: String, nc3rsrp: String, ncci4: String, nc4rsrp: String,
      ncci5: String, nc5rsrp: String, lon: Double, lat: Double )
  ...
  def rddToMroDFCase(sparkSession : SparkSession, path:String):Dataset[Mro] = {
    ...
    val mroRDD = rowsRDD.map(row => row.split(",")).filter(_.size == 28)
                    .map{col => Mro(col(0),col(1),col(2),col(5)
                        ,col(6),col(9),col(10),col(13),col(14),col(17),col(18),col(21),col(22),col(25)
                        ,col(26).toDouble,col(27).toDouble)}
  ...
  def rddToTrainingMroDFCase(sparkSession : SparkSession, path:String):Dataset[TrainingMro] = {
    val rowsRDD = sparkSession.sparkContext.textFile(path)
//    val mroRDD = rowsRDD.map(row => row.split(",")).filter(_.size == 15)
//                        .map{col => TrainingMro(col(3),col(6).toDouble,col(7),col(10).toDouble,col(12).toDouble,col(11).toDouble)}
...
//    val testdataDir="data/test_payload.txt"
//    val traindataDir = "data/train_payload.txt"
...
//    
    ...
    val testdatasql = "select objid,scrsrp,timestamp,s1apid,substr(rpad(lon,8,'0'),1,8) as lon,substr(rpad(lat,7,'0'),1,7) as lat,"+
      "concat_ws('-', case when ncci1='' then null else ncci1 end,case when ncci2='' then null else ncci2 end,case when ncci3='' then null else ncci3 end,case when ncci4='' then null else ncci4 end,case when ncci5='' then null else ncci5 end) as ncci_list,"+
      "concat_ws('-', case when nc1rsrp='' then null else nc1rsrp end,case when nc2rsrp='' then null else nc2rsrp end,case when nc3rsrp='' then null else nc3rsrp end,case when nc4rsrp='' then null else nc4rsrp end,case when nc5rsrp='' then null else nc5rsrp end) as ncrsrp_list from mro_testdata"
    val mrotest1DF = sparkSession.sql(testdatasql)
